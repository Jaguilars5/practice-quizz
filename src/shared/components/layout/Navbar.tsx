import { logout } from "@auth/services";
import { useAuthStore } from "@auth/store";
import { Button } from "@shared/components/ui/Button";
import { AnimatePresence, motion } from "framer-motion";
import {
  BookOpen,
  ClipboardList,
  LogOut,
  Menu,
  Plus,
  Trophy,
  User,
  X,
} from "lucide-react";
import { useState } from "react";
import { Link, useLocation } from "react-router-dom";

import { ROUTES } from "@app/constants/routes";

export const Navbar = () => {
  const { user } = useAuthStore();
  const { pathname } = useLocation();
  const [menuOpen, setMenuOpen] = useState(false);

  const isActive = (path: string) =>
    pathname === path || (path !== ROUTES.HOME && pathname.startsWith(path));

  const linkClass = (path: string) =>
    `transition-colors text-sm font-medium ${isActive(path) ? "text-primary-300" : "text-gray-300 hover:text-white"}`;

  const navLinks = user
    ? [
        { to: ROUTES.CREATOR, label: "Crear", icon: Plus },
        { to: ROUTES.FLASHCARDS, label: "Flashcards", icon: BookOpen },
        { to: ROUTES.MY_ANSWERS, label: "Respuestas", icon: ClipboardList },
        { to: ROUTES.LEADERBOARD, label: "Ranking", icon: Trophy },
      ]
    : [];

  return (
    <motion.nav
      initial={{ y: -80 }}
      animate={{ y: 0 }}
      transition={{ type: "spring", stiffness: 200, damping: 25 }}
      className="fixed top-0 left-0 right-0 z-40 bg-gray-950/80 backdrop-blur-xl border-b border-gray-800"
    >
      <div className="max-w-6xl mx-auto px-3 sm:px-4 h-14 sm:h-16 flex items-center justify-between">
        <Link
          to={ROUTES.HOME}
          className="flex items-center gap-2"
          onClick={() => setMenuOpen(false)}
        >
          <img src="/src/assets/logo.svg" alt="QuizzY" className="h-8 w-8" />
          <motion.span
            whileHover={{ scale: 1.05 }}
            className="text-xl font-bold text-white inline-block"
          >
            <span className="text-primary-400">Quiz</span>zY
          </motion.span>
        </Link>

        <div className="flex items-center gap-1 sm:gap-4">
          {user ? (
            <>
              <div className="hidden sm:flex items-center gap-4">
                <Link to={ROUTES.CREATOR} className={linkClass(ROUTES.CREATOR)}>
                  Crear
                </Link>
                <Link
                  to={ROUTES.FLASHCARDS}
                  className={`${linkClass(ROUTES.FLASHCARDS)} flex items-center gap-1`}
                >
                  <BookOpen size={16} />
                  <span className="hidden sm:inline">Flashcards</span>
                </Link>
                <Link
                  to={ROUTES.MY_ANSWERS}
                  className={`${linkClass(ROUTES.MY_ANSWERS)} flex items-center gap-1`}
                >
                  <ClipboardList size={16} />
                  <span className="hidden sm:inline">Mis respuestas</span>
                </Link>
                <Link
                  to={ROUTES.LEADERBOARD}
                  className={linkClass(ROUTES.LEADERBOARD)}
                >
                  Ranking
                </Link>
              </div>

              <div className="flex items-center gap-2 text-sm text-gray-400">
                {user.photoURL ? (
                  <img
                    src={user.photoURL}
                    alt=""
                    className="w-7 h-7 rounded-full"
                  />
                ) : (
                  <User size={18} />
                )}
                <span className="hidden sm:inline max-w-[100px] truncate">
                  {user.displayName}
                </span>
              </div>

              <Button
                variant="ghost"
                size="sm"
                onClick={logout}
                className="hidden sm:inline-flex"
              >
                <LogOut size={16} />
              </Button>

              <button
                onClick={() => setMenuOpen(!menuOpen)}
                className="sm:hidden p-2 text-gray-300 hover:text-white"
              >
                {menuOpen ? <X size={22} /> : <Menu size={22} />}
              </button>
            </>
          ) : (
            <Link to={ROUTES.LOGIN}>
              <Button size="sm">Ingresar</Button>
            </Link>
          )}
        </div>
      </div>

      <AnimatePresence>
        {menuOpen && user && (
          <motion.div
            initial={{ opacity: 0, height: 0 }}
            animate={{ opacity: 1, height: "auto" }}
            exit={{ opacity: 0, height: 0 }}
            className="sm:hidden border-t border-gray-800 bg-gray-950/95 backdrop-blur-xl overflow-hidden"
          >
            <div className="px-4 py-3 space-y-1">
              {navLinks.map((l) => (
                <Link
                  key={l.to}
                  to={l.to}
                  onClick={() => setMenuOpen(false)}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm ${isActive(l.to) ? "bg-primary-500/10 text-primary-300" : "text-gray-300 hover:bg-gray-800"}`}
                >
                  <l.icon size={18} />
                  {l.label}
                </Link>
              ))}
              <button
                onClick={() => {
                  logout();
                  setMenuOpen(false);
                }}
                className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-gray-300 hover:bg-gray-800 w-full"
              >
                <LogOut size={18} />
                Cerrar sesión
              </button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
    </motion.nav>
  );
};
