...
import { getLocalFlashcardSets, saveFlashcardSetToLocal, getFlashcardSetsByCreator, getGlobalFlashcardSets, getFlashcardSetByCode, getLocalFlashcardSetByCode, getFlashcardSetsByFolderId, deleteFlashcardSetFromFirestore, deleteFlashcardSetLocal } from '../firebase/flashcardService'
...
import { Plus, BookOpen, Search, AlertCircle, Loader2, FolderOpen, ArrowUpDown, Globe, KeyRound, ClipboardPaste } from 'lucide-react'
import type { FlashcardSet, Folder } from '../types'
...
export const FlashcardsHome = () => {
  ...
  const handleSearch = async () => {
    ...
    if (hasFirebaseConfig) {
      ...
      } catch { setSearchError('Error al buscar'); setSearching(false); return }
    }
  ...
  const handleDelete = async () => {
    ...
    if (hasFirebaseConfig) {
      try { await deleteFlashcardSetFromFirestore(s.code) } catch { /* fallback */ }
    }
  ...
  const handleJoinFolder = async () => {
    ...
    if (hasFirebaseConfig) {
      ...
      } catch { setFolderSearchError('Error al buscar'); setFolderSearching(false); return }
    }
