import { generateCode } from "@quiz/constants/quiz.constants";
...
export const questionSchema = yup.object({
  ...
  type: yup
    .mixed<"multiple" | "truefalse">()
    .oneOf(["multiple", "truefalse"])
    .required(),
  options: yup.array().when("type", {
    is: "multiple",
    then: (schema) =>
      schema
        .of(yup.string().required("Opción obligatoria"))
...
export const testSchema = yup.object({
  ...
  difficulty: yup
    .mixed<"facil" | "medio" | "dificil">()
    .oneOf(["facil", "medio", "dificil"])
    .required(),
  timePerQuestion: yup
    .number()
    .min(5, "Mínimo 5 segundos")
