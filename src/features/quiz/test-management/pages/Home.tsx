...
import { TestCard } from "@quiz/test-management/components/TestCard";
import { useFolderManager } from "@quiz/test-management/hooks/useFolderManager.hook";
import { useSharedFolderTests } from "@quiz/test-management/hooks/useSharedFolderTests.hook";
import { useTestData } from "@quiz/test-management/hooks/useTestData.hook";
import { useTestFilter } from "@quiz/test-management/hooks/useTestFilter.hook";
import { useTestSearch } from "@quiz/test-management/hooks/useTestSearch.hook";
import {
...
export const Home = () => {
  ...
  const { tests, folders, refreshTests, refreshFolders } = useTestData(
    user?.email,
  );
  ...
  const sharedFolderTests = useSharedFolderTests(
    activeFolder,
    folders,
    user?.email,
  );
  const {
    ...
    searchTitle,
    setSearchTitle,
  } = useTestFilter(tests, sharedFolderTests, activeFolder);

  const handleTestFound = () => {
    refreshTests();
  };

  const { searchCode, setSearchCode, searching, searchError, handleSearch } =
    useTestSearch(handleTestFound);
  ...
  const handleImport = (test: Test) => {
    navigate(ROUTES.CREATOR, { state: { editTest: test } });
  };
  ...
  const handleDelete = async () => {
    ...
    if (hasFirebaseConfig) {
      try {
        ...
        if (found) await deleteTestFromFirestore(found.id);
      } catch (error) {
        logError(error, "Home:handleDelete");
      }
    }

    deleteTestLocal(test.id);
    refreshTests();
        ...
        >
          <div className="space-y-4">
            <p className="text-gray-300">
              ¿Eliminar <strong>{deleteConfirm.title}</strong>? Esta acción no
              se puede deshacer.
            </p>
      ...
      <div className="flex flex-col md:flex-row gap-4 md:gap-6">
        <div className="md:w-48 shrink-0">
          <details className="md:hidden">
            <summary className="text-sm text-gray-400 cursor-pointer mb-2 select-none">
              Carpetas ▸
            </summary>
            <FolderSidebar
              ...
              handleRenameFolder={(id, name) =>
                handleRenameFolder(id, name, refreshFolders)
              }
              handleDeleteFolder={(id) =>
                handleDeleteFolder(id, refreshTests, refreshFolders)
              }
          ...
          {filteredTests.length === 0 ? (
            ...
            >
              ...
              <p className="text-sm mt-1">
                Crea, importa o busca un test por código para empezar
              </p>
            </motion.div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {filteredTests.map((test, i) => (
                ...
                >
                  <TestCard
                    test={test}
                    onPlay={() => navigate(ROUTES.PLAY(test.id))}
                    onEdit={
    ...
    <div className="border-t border-gray-700 pt-2">
      <div className="flex gap-1 min-w-0">
        ...
        <Button
          ...
          disabled={folderSearching || !folderSearchCode.trim()}
          className="shrink-0"
        >
          {folderSearching ? (
      ...
      {folderSearchError && (
        <p className="text-xs text-red-400 mt-1">{folderSearchError}</p>
      )}
    </div>
  </Card>
);
