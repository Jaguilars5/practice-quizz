...
import { getLocalFolders, createFolder, renameFolder, deleteFolder, getFolderByCodeLocal, getFolderByCodeFromFirestore, updateFolderVisibility } from '@folders/services/folders.api'
...
interface UseFolderManagerReturn {
  ...
  handleUpdateFolderVisibility: (id: string, visibility: Folder['visibility'], code?: string) => Promise<void>
  handleDeleteFolder: (id: string, onRefreshTests: () => void, onRefreshFolders: () => void) => Promise<void>
  handleRenameFolder: (id: string, name: string, onRefreshFolders: () => void) => Promise<void>
}
...
export const useFolderManager = (
  ...
  const handleCreateFolder = useCallback(async (userEmail: string) => {
  ...
  const handleUpdateFolderVisibility = useCallback(async (id: string, visibility: Folder['visibility'], code?: string) => {
    await updateFolderVisibility(id, visibility, code)
    refreshFolders()
  }, [refreshFolders])

  const handleDeleteFolder = useCallback(async (id: string, onRefreshTests: () => void, onRefreshFolders: () => void) => {
  ...
  const handleRenameFolder = useCallback(async (id: string, name: string, onRefreshFolders: () => void) => {
  ...
  return {
    ...
    folderSearchCode, setFolderSearchCode,
    folderSearching, folderSearchError,
    handleCreateFolder,
    handleJoinFolder,
