...
import { getAnswersByPlayer, getLocalAnswers } from '@quizData'
...
import { ArrowLeft, ClipboardList, Play, Trash2, ArrowUpDown, Search } from 'lucide-react'
import type { AnswerSet } from '@shared/types'

const AnswerCard = ({ answer, onPlay, onRemove }: { answer: AnswerSet; onPlay: () => void; onRemove: () => void }) => {
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            <h3 className="text-lg font-bold text-white truncate">{answer.testTitle}</h3>
            ...
            <div className="flex flex-wrap items-center gap-2 mt-3">
              ...
              <Badge color="gray">{answer.stats.correct}/{answer.stats.correct + answer.stats.incorrect}</Badge>
              <Badge color="gray">{answer.stats.avgTime.toFixed(1)}s prom.</Badge>
            </div>
          </div>
          <div className="flex gap-2 shrink-0 items-start">
            <Button size="sm" onClick={(e) => { e.stopPropagation(); onPlay() }}>
              <Play size={14} />
            </Button>
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {[...answers]
            .filter(a => !searchTitle.trim() || a.testTitle.toLowerCase().includes(searchTitle.toLowerCase()))
            .sort((a, b) =>
            sortNewest
