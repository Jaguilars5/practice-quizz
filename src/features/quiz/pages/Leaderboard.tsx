...
import { getLocalTests } from '@quizData'
import { getAnswersByTest, getLocalAnswers } from '@quizData'
...
import { ArrowLeft, Trophy, Medal, Search } from 'lucide-react'
import type { Test, AnswerSet } from '@shared/types'
...
export const Leaderboard = () => {
  ...
  const rankings = useMemo(() => {
    const grouped = answers.reduce<Record<string, { name: string; score: number; correct: number; total: number; count: number }>>((acc, a) => {
      if (!acc[a.playerId]) {
        acc[a.playerId] = { name: a.playerName, score: 0, correct: 0, total: 0, count: 0 }
      }
      ...
      {tests.length === 0 ? (
        ...
        <>
          ...
          <div className="flex gap-2 flex-wrap">
            {tests.filter(t => !searchTitle.trim() || t.title.toLowerCase().includes(searchTitle.toLowerCase())).map(t => (
              <button
                key={t.id}
          ...
          {!selectedTestId ? (
            ...
            <div className="text-center py-20 text-gray-500">
              <p>No hay respuestas para <strong>{selectedTest?.title}</strong> todavía</p>
            </div>
          ) : (
                ...
                >
                  <Card className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <span className="text-white font-semibold truncate block">{entry.name}</span>
                      <div className="flex gap-2 mt-1">
                        <Badge color="primary">{entry.score} pts</Badge>
                        <Badge color="gray">{entry.correct}/{entry.total} correctas</Badge>
                        <Badge color="gray">{entry.count} intento{entry.count !== 1 ? 's' : ''}</Badge>
                      </div>
                    </div>
                  </Card>
                </motion.div>
