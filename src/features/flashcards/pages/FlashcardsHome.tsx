...
import { FlashcardCard } from "@flashcards/components/FlashcardCard";
import {
  deleteFlashcardSetFromFirestore,
  deleteFlashcardSetLocal,
  getFlashcardSetByCode,
  getFlashcardSetsByCreator,
  getFlashcardSetsByFolderId,
...
export const FlashcardsHome = () => {
  ...
  const handleSearch = async () => {
    ...
    if (local) {
      navigate(`/flashcards/study/${local.id}`);
      return;
    }
  ...
  const handleDelete = async () => {
    ...
    if (hasFirebaseConfig) {
      try {
        await deleteFlashcardSetFromFirestore(s.code);
      } catch (error) {
  ...
  const handleUpdateFolderVisibility = async (
    id: string,
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-gray-400 mt-1 text-sm sm:text-base">
            Estudiá con flashcards
          </p>
        </div>
        <div className="flex gap-2 flex-wrap">
          <Button
            size="sm"
            onClick={() => navigate("/flashcards/crear")}
            ...
            <Card className="p-3 space-y-3">
              ...
              {folders.length > 0 && (
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all mt-1 ${activeFolder === "__uncategorized" ? "bg-primary-500/10 text-primary-300" : "text-gray-400 hover:text-white hover:bg-gray-800/50"}`}
                >
                  <FolderOpen size={16} />
              ...
              <div className="border-t border-gray-700 pt-2">
                <div className="flex gap-1 min-w-0">
                  <input
                    ...
                    onChange={(e) =>
                      setFolderSearchCode(e.target.value.toUpperCase())
                    }
          ...
          {filteredSets.length === 0 ? (
            ...
            >
              ...
              <p className="text-sm mt-1">
                Creá un set o buscá por código para empezar
              </p>
            </motion.div>
          ) : (
