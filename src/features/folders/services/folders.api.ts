...
export const createFolder = async (
  ...
  if (db) {
    try {
      ...
      logError(error, 'folders.api:createFolder')
...
export const renameFolder = async (id: string, name: string) => {
  if (db) {
    try {
      ...
      logError(error, 'folders.api:renameFolder')
    }
  }
  const folders = getLocalFolders()
...
export const updateFolderVisibility = async (id: string, visibility: 'private' | 'public' | 'code', code?: string) => {
  if (db) {
    try {
      ...
      logError(error, 'folders.api:updateFolderVisibility')
    }
  }
  const folders = getLocalFolders()
  const idx = folders.findIndex(f => f.id === id)
...
export const deleteFolder = async (id: string) => {
  if (db) {
    try {
      ...
      logError(error, 'folders.api:deleteFolder')
    }
  }
